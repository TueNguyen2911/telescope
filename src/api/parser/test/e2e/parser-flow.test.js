...
const { hash, fetch, Satellite } = require('@senecacdot/satellite');
...
describe("Testing parser service's flow", () => {
  test('Parser should process 2 valid feeds', async () => {
    ...
    const all_feeds = await fetchData(`${process.env.POSTS_URL}/feeds`);
    const valid_feeds = await Promise.all(all_feeds.map((feed) => fetchData(feed.url)));
    const valid_ids = valid_feeds.map((feed) => feed.id);

    expect(all_feeds.length).toBeGreaterThan(0);
    expect(
      checker(
        valid_ids,
  ...
  test('Parser should process 2 invalid feeds', async () => {
    ...
    const invalid_feeds = await Promise.all(feeds.map((feed) => fetchData(feed.url)));
    const invalid_ids = invalid_feeds.map((feed) => feed.id);

    expect(invalid_feeds.length).toBeGreaterThan(0);
    expect(
      checker(
        invalid_ids,
